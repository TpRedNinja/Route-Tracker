...
public override Dictionary GetStatsAsDictionary()

{

// Use ReadWithCache for better performance (inherited from GameStatsBase)

int ravens = ReadWithCache("ravens", (nint)baseAddress, ravenOffsets);

int chests = ReadWithCache("chests", (nint)baseAddress, chestOffsets);

int artifacts = ReadWithCache("artifacts", (nint)baseAddress, artifactOffsets);
...
Artifact #1Artifact1Midgard - Lookout Tower

Legendary Chest #1Chest2Midgard - Witch's Cave
...
"Viewpoint" or "viewpoint" => stats.GetValue("Viewpoints", 0) >= entry.Condition,

"Chest" or "chest" => stats.GetValue("Chests", 0) >= entry.Condition,

// God of War 2018 types

"Raven" or "raven" => stats.GetValue("Ravens", 0) >= entry.Condition,

"Artifact" or "artifact" => stats.GetValue("Artifacts", 0) >= entry.Condition,
